...
class TaskOccurrenceTest < ActiveSupport::TestCase
  ...
  test "completed method for a task that should be checked" do
    Timecop.freeze(Time.now) do
      task_occurrence = create :task_occurrence_should_be_checked
      assert !task_occurrence.completed?
      
      task_occurrence.checked = true
  ...
  test "completed method for a task that should not be checked" do
    Timecop.freeze(Time.now) do
      task_occurrence = create(:task_occurrence)
      assert !task_occurrence.completed? 

      task_occurrence = create(:task_occurrence_passed_deadline)
      assert task_occurrence.completed?
    end
  end
  ...
  test "allocate in turns" do
    ...
    task = create(:task_with_occurrences, 
  ...
  test "allocate in turns with ignored user" do
    community = create(:community_with_users) # 5 users
  ...
  test "todo scope: should not be checked" do
    ...
    task_occurrence.deadline = 2.weeks.ago
    task_occurrence.save
  ...
  test "allocate by time" do
    community = create(:community_with_users, users_count: 2)
    ...
    2.times {create(:task_occurrence, task: task, user: user1)}
  ...
  test "allocate by time on all tasks within community" do
    ...
    task1 = create(:task_with_occurrences, 
                    allocation_mode: 'time',
                    community: community,
                    occurrences_user: user2,
                    occurrences_count: 3
    ...
    task2 = create(:task_with_occurrences, 
    ...
    2.times {create(:task_occurrence, task: task1, user: user1)}
  ...
  test "send a reminder mail" do
    @user = create(:user)
    @task_occurrences = 2.times.map {create(:task_occurrence, deadline: 2.days.since, user: @user)}
  ...
  test "reminder mail completed task" do
    @user = create(:user)
    @task_occurrence = create(:task_occurrence, deadline: 1.days.since, user: @user, should_be_checked: true, checked: true)
