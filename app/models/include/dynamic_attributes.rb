module DynamicAttributes
  def evaluate_dynamic_attributes
    self.dynamic_attribute_values = self.class::PERSIST_DYNAMIC_ATTRIBUTES.inject({}) do |dynamic_attribute_values, attribute|
      dynamic_attribute_values.merge! attribute => self.send(attribute)
    end
  end
  def populate_dynamic_attributes
    (self.dynamic_attribute_values || {}).each do |attribute, value|
